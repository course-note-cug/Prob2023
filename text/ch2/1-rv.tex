...
\begin{example}
    ...
    \begin{tabular}{c|c|c|c|c}
        ...
        b) 随机变量 $X$ 的另一简单的例子是某集合 $A \in \mathscr{A}$ 的\newword{示性函数}{indicator function} (亦称特征函数):
        $$
        X=I_A(\omega)
        $$其中, $$
...
$$
其中组数 $\left\{P_{X}\left(x_1\right), \cdots, P_{X}\left(x_m\right)\right\}$ 称做随机变量 $X$ 的\newword{概率分布}{probability distribution}.

随机变量 $X$ 的构造完全由概率分布 $\left\{P_{X}\left(x_i\right), 1,2, \cdots, m\right\}$ 描述. 下面引进的\newword{分布函数}{probability}的概念, 提供随机变量构造的等价描述.
...
\begin{definition}[\newword{随机变量}{random variable, r.v.}]
