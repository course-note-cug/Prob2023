...
\begin{theorem}[\newword{乘法公式}{formula of multiplication}]
    由条件概率的定义, 得到
    \begin{itemize}
...
\begin{theorem}[\newword{全概率公式}{Formula for total probability}]
...
\end{corollary}
    ...
    实际上, 在统计应用中, 事件 $A_1, \cdots, A_n$ 组成事件组 $\left(A_1+\cdots+A_n=\Omega\right)$, 常称做 ``假设'' 或 ``假说'', 而 $P\left(A_i\right)$ 称做假设 $A_i$ 的\newword{先验}{prior}概率 $\left.\right)$. 条件概率 $P\left(A_i \mid B\right)$ 称做假设 $A_i$ 在事件 $B$ 出现后的\newword{后验}{posterior}概率. 
