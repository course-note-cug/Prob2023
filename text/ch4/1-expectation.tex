...
可能我们的第一考虑是看看``平均''能得多少. 这样的随机变量$X$, \[
    ...
    绝对收敛, 则称级数 $\sum_{k=1}^{\infty} x_k p_k$ 的和为随机变量 $X$ 的\newword{数学期望}{mathematical expectation}, 记为 $\Ep{X}$. 即
    $$
...
这个为什么结果如此简单? 实际上因为期望有一些性质. 我们下一节会提到原因. 
    ...
    有些分布没有数学期望. 比如\newenword{Cauchy分布}: $f_X(x)=\frac{4}{\pi} \frac{4}{1+x^2},-\infty<x<+\infty$. 我们在求它的积分的时候, 就会发现
    $$
\begin{aligned}
