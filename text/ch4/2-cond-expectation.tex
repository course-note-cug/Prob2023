\section{条件数学期望}
    ...
    \begin{definition}
        ...
        $$
        若级数$\sum_i x_i p_{i|j}$绝对收敛, 那么称此级数$Y=Y_i$条件下$X$的\newword{条件数学期望}{conditional mathematical expectation}, 记为$\Ep{X|Y=y_i}$.
    \end{definition}
...
上面的式子经过整理得到$\Ep{X}=p \cdot 1+(1-p)(\Ep{X}+1)$. 我们由此解答出来$\Ep{X}$, 因此就得到期望值为$1/p$. 
    ...
    这个公式有一个名字, 叫做``\newword{迭代期望定律}{Law of iterated expectations}''. 
