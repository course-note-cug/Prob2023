...
\begin{example}
    ...
    $$
    $(i=1,2, \cdots)$, 则 $X_1, X_2, \cdots$ 是随机变量序列. 所谓 $n$ 次独立试验, 就是指随机变量 $X_1, \cdots, X_n$ 相互独立. 显然 $A$ 发生的次数 $\mu_n=\sum_{i=1}^n X_i$. 记 $X_n=\frac{1}{n} \sum_{i=1}^n X_i(n \geq 1)$. 问题是: 当 $n$ 很大时, 随机变量 $X_n$ 是否与常数 $P(A)$ 很接近?

    我们可以对于任何随机变量 $X$, 我们可以使用比较简单的离散型随机变量 $X^*$ 来近似 $X, 0 \leq X-X^*<\varepsilon$, 这里我们选取比较简单的分段函数$X^*$ , 它与 $\varepsilon$ 有关.$$
    X^*= \begin{cases}0, & 0 \leq X<\varepsilon, \\ -\varepsilon, & -\varepsilon \leq X<0, \\ \varepsilon, & \varepsilon \leq X<2 \varepsilon, \\ \cdots \cdots & \cdots \cdots \\ k \varepsilon, & k \varepsilon \leq X<(k+1) \varepsilon \quad(k \text { 是任何整数) }, \\ \cdots \cdots & \cdots \cdots\end{cases}
    $$
...
\begin{definition}
    称 $X_1, X_2, \cdots$ \newword{依概率收敛}{convergence in probability}于 $\eta$, 若对任何 $\varepsilon>0$, 成立
$$
...
\begin{definition}
    称 $X_1, X_2, \cdots$ 概率为 1 (或\newword{几乎必然收敛}{almost surely converges to}) 地收敛于 $\eta$, 若
$$
...
\begin{definition}
    称 $X_1, X_2, \cdots$ \newword{弱收敛}{weak converge}(\newword{依分布收敛}{converge in distribution})于 $\eta$, 若对 $\eta$ 的分布函数 $F(x)$ 的任何连续点 $x$, 皆成立
    $$
...
\begin{example}
    设 $\Omega=(0,1), \mathscr{F}$ 由 $(0,1)$ 中所有 Borel 子集组成, $P$ 是这样的概率测度: 对任何区间 $(a, b)(0 \leq a<b \leq 1), P((a, b))=b-a$. 在概率空间 $(\Omega, \mathscr{F}, P)$ 上考虑下列随机变量序列.
...
0, & \text { 其他; }\end{cases} \\
X_{k j} & = \begin{cases}1, & \frac{j-1}{2^k} \leq \omega<\frac{j}{2^k}, \quad(j>1) . \\
0, & \text { 其他 }\end{cases}
\end{aligned}
$$

这些 $\left\{X_{k j}: k \geq 1, j=1,2, \cdots, 2^k\right\}$ 可排成一个序列: $X_{11}, X_{12}, X_{21}, X_{22}$, $X_{23}, X_{24}, X_{31}, X_{32}, \cdots, X_{38}, X_{41}, \cdots$ (用 ``字典排列法'', 按第 1 个下标 $k$ 从小到大排, 第 1 个下标相同者则按第 2 个下标从小到大排). 把这个序列依次记为 $X_1, X_2, \cdots$. 易知, 对每个 $n \geq 1$ 有 $k_n$ 和 $j_n$ 使得 $X_n=X_{k_n j_n}$ 对任何 $\varepsilon \in(0,1), P\left(\left|X_n\right| \geq \varepsilon\right)=P\left(X_n=1\right)=\frac{1}{2^{k_n}}$. 由于 $n \rightarrow \infty$ 时 $k_n \rightarrow \infty$, 故有 $\lim _{n \rightarrow \infty} P\left(\left|X_n\right| \geq \varepsilon\right)=0$. 这表明 $X_n \stackrel{P}{\longrightarrow} 0$.
